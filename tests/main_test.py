...
def test_get_all_posts_json():  # тест проверяет все посты, что
    ...
    keys_list = sorted(['poster_name', 'poster_avatar', 'pic', 'content', 'views_count', 'likes_count', 'pk'])  # проверочный список ключей
...
def test_get_postid_json():  # тест проверяет конкретный пост, что
    ...
    #2) у элементов есть нужные ключи
    keys_list = sorted(['poster_name', 'poster_avatar', 'pic', 'content', 'views_count', 'likes_count', 'pk'])
